...
function generateMetric(projectId?: string): Record<string, unknown> {
  const metric = {
    ...
    metric_name: CLAUDE_CODE_METRICS[Math.floor(Math.random() * CLAUDE_CODE_METRICS.length)],
    value: Math.floor(Math.random() * 10000) + 100,
    app_version: '1.0.0',
    attributes: {
      type: ['input', 'output', 'cacheRead'][Math.floor(Math.random() * 3)] as 'input' | 'output' | 'cacheRead',
...
function generateEvent(projectId?: string): Record<string, unknown> {
  const event = {
    event_name: EVENT_NAMES[Math.floor(Math.random() * EVENT_NAMES.length)] as typeof EVENT_NAMES[number],
    timestamp: new Date().toISOString(),
    session_id: `session-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    attributes: {
...
function generateGAEvent(projectId?: string): Record<string, unknown> {
  const events = [
    { name: 'page_view', params: { page_location: 'https://example.com', page_title: 'Home' } },
    { name: 'click', params: { element_id: 'button-123', element_text: 'Click me' } },
    { name: 'scroll', params: { percent_scrolled: Math.floor(Math.random() * 100) } },
    { name: 'purchase', params: { value: (Math.random() * 100).toFixed(2), currency: 'USD' } },
  ];
 ...
 */
async function sendEvent(endpoint: string, data: Record<string, unknown>, type: 'cc' | 'ga'): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(`✅ Sent ${type} event:`, data.metric_name || data.event_name || (data.events as { name: string }[])?.[0]?.name);
    }
  } catch (error) {
...
function parseArgs(): EventOptions {
  ...
  let endpoint = 'http://localhost:8788'; // Default local dev server
...
Options:
  --remote, -r              Send to production (https://logs.duyet.net)
