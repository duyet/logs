...
export async function createProject(
  ...
  if (!isValidProjectId(id)) {
    throw new Error('Invalid project ID format. Must be 3-32 alphanumeric characters.');
  }
  ...
  // Insert project
  await db
    .prepare('INSERT INTO projects (id, description, created_at, last_used) VALUES (?, ?, ?, NULL)')
...
export async function getProject(db: D1Database, id: string): Promise<Project | null> {
  const result = await db
    .prepare('SELECT id, description, created_at, last_used FROM projects WHERE id = ?')
    .bind(id)
    .first<Project>();
...
export async function listProjects(
  ...
  const { results } = await db
    .prepare('SELECT id, description, created_at, last_used FROM projects ORDER BY created_at DESC LIMIT ? OFFSET ?')
    .bind(limit, offset)
    .all<Project>();
