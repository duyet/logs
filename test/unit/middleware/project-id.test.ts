...
import { projectIdMiddleware, extractProjectId } from '../../../src/middleware/project-id.js';
...
describe('projectIdMiddleware', () => {
  ...
  it('should validate existing project_id and update last_used', async () => {
    ...
    const updateLastUsedSpy = vi.spyOn(projectService, 'updateLastUsed').mockResolvedValue();
    ...
    expect(projectService.projectExists).toHaveBeenCalledWith(mockDB, 'existing123');
  ...
  it('should handle validation errors gracefully', async () => {
    ...
    vi.spyOn(projectService, 'projectExists').mockRejectedValue(new Error('DB error'));
  ...
  it('should handle updateLastUsed errors without blocking', async () => {
    ...
    vi.spyOn(projectService, 'projectExists').mockResolvedValue(true);
    vi.spyOn(projectService, 'updateLastUsed').mockRejectedValue(new Error('Update failed'));
